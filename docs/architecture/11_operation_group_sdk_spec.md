...
本書は実装前の変更仕様である。rv-metaの[Operation Group / Function Operation / OpenAPI変更仕様](../../../rv-meta/00_docs/operation_group_openapi_spec.md)を前提とする。

実装は次の順序で分離する。

1. Operation Group / Function Operation表示
2. OpenAPI検証
3. SDK Generator Port / Adapter
4. SDK生成UI

画面、状態管理、IPC、Rust、外部Generator呼び出しを一度に変更しない。
...
- Operationは`entityId`必須で、Entity詳細からしか辿れない。
- Document詳細にはEntity一覧しかない。
- Rustの`MetadataService`がSQLを直接実行しており、Repository以外にDBアクセスを書かない規則と一致していない。
- SDK出力の表示文言はあるが、検証・Generator境界・出力結果モデルがない。
- UIの`operationId`というRoute値が、OpenAPIの文字列`operationId`ではなくDB行IDを指しており意味が曖昧である。
...
get_operation_group_detail(operation_group_id)
...
```text
get_operation_groups(schema?: string)
get_operation_group_detail(operation_group_id: int)
...
```text
operationGroups
operationGroupDetail(operationGroupId)
operationDetail(operationRowId, backRoute)
