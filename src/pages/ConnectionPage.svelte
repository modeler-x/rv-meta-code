...
<div class="mb-2 flex items-center px-1">
  ...
  <button class="flex items-center gap-1 rounded-md bg-[color:var(--rvc-accent)] px-3 py-1 text-xs font-semibold text-white" data-testid="add-connection" onclick={() => viewModel.startAdd()}><Plus size={13} /> {$t('c_add')}</button>
</div>
...
{#if viewModel.isFormOpen}
  ...
  <div class="mb-6 flex gap-2">
    ...
    <button class="rounded-md border border-[color:var(--rvc-border)] px-4 py-2 disabled:opacity-50" disabled={viewModel.testState === 'testing'} data-testid="test-connection" onclick={() => viewModel.testConnection()}>{viewModel.testState === 'testing' ? $t('c_testing') : $t('c_test')}</button>
...
<SectionList title="">
  {#each viewModel.connections as connection}
    <SectionListRow isButton on:click={() => viewModel.startEdit(connection.id)}>
      <span data-testid="connection-row" data-connection={connection.id} data-current={connection.isCurrent} class="contents">
      ...
      {#if !connection.isCurrent}<button class="text-xs text-[color:var(--rvc-accent)]" data-testid="set-active" data-connection={connection.id} onclick={(event) => { event.stopPropagation(); viewModel.setCurrentConnection(connection.id); }}>{$t('c_set_active')}</button>{/if}
