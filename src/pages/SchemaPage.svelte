<script lang="ts">
  ...
  export let viewModel: SchemaViewModel;
  export let generationViewModel: GenerationViewModel;
</script>
...
<SectionList title={`${$t('sec_schemas')} / ${viewModel.state.schemas.length}`} detail={$t('schemas_hint')}>
  {#each viewModel.state.schemas as schema}
    <SectionListRow isButton>
      ...
      <span class="min-w-0 flex-1"><span class="block font-mono font-semibold">{schema.name}</span><span class="block text-xs text-[color:var(--rvc-muted)]">{schema.description}</span></span>
      <span class="text-xs text-[color:var(--rvc-muted)]">{schema.tableCount} {$t('unit_tables')} / {schema.viewCount} {$t('unit_views')}</span>
      <button class="rounded-md bg-[color:var(--rvc-accent)] px-3 py-1.5 text-xs font-semibold text-white" on:click|stopPropagation={() => generationViewModel.askGeneration(schema)}>{$t('generate')}</button>
    </SectionListRow>
  {/each}
</SectionList>
