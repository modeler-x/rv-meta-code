...
export const messages = {
  en: {
    ...
    sv_connectivity: 'Connectivity check', sv_health_path: 'Health path', sv_expected_status: 'Expected status', sv_timeout_ms: 'Timeout (ms)', sv_base_override: 'Absolute base URL', sv_connectivity_hint: 'Resolves server variables with their defaults, then GETs base URL + health path with the timeout, and checks the expected status. Set an absolute base URL when the server URL is relative.', sv_test: 'Test connectivity', sv_testing: 'Testing…',
  ...
  ja: {
    ...
    sv_connectivity: '導通確認', sv_health_path: 'ヘルスパス', sv_expected_status: '期待ステータス', sv_timeout_ms: 'タイムアウト (ms)', sv_base_override: '絶対ベースURL', sv_connectivity_hint: 'サーバー変数を default で解決し、ベースURL＋ヘルスパスへタイムアウト付きで GET し、期待ステータスと照合します。ベースURLが相対の場合は絶対ベースURLを指定してください。', sv_test: '導通確認', sv_testing: '確認中…',
